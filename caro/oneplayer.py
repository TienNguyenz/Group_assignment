...
def play_game_until_result(driver):
    """Chơi game tự động đến khi có kết quả hoặc tự động reset khi gần hòa."""
    while True:
...
@pytest.mark.parametrize("i", range(10))  # Lặp lại 10 lần
def test_draw(driver,i):
    """Test trường hợp hòa giữa người chơi (X) và bot (O)."""
    
    # Người chơi (X) và bot (O) lần lượt đi, tạo ra một trận hòa
    # Người chơi (X) đi vào các ô không tạo cơ hội thắng
    make_move(driver, 5)  # Người chơi đánh vào ô 1
    time.sleep(1)
    # Bot tự động đánh theo thuật toán Minimax mà không cần can thiệp
    # Chờ cho bot đánh tự động theo nước đi tối ưu
    result = play_game_until_result(driver)
